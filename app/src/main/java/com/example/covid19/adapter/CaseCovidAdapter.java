...
public class CaseCovidAdapter extends ListAdapter<DataCovid, CaseCovidAdapter.ViewHolder> {

    private final DataCovidClickableCallback callback;

    public CaseCovidAdapter(@NonNull DataCovidClickableCallback callback) {
        super(new AsyncDifferConfig.Builder<>(new DiffUtil.ItemCallback<DataCovid>() {
            @Override
            public boolean areItemsTheSame(@NonNull DataCovid oldItem, @NonNull DataCovid newItem) {
                return oldItem.country.equalsIgnoreCase(newItem.country);
            }
            @Override
            public boolean areContentsTheSame(@NonNull DataCovid oldItem, @NonNull DataCovid newItem) {
                return oldItem.country.equalsIgnoreCase(newItem.country);
            }
        }).build());
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener{
        ...
        public void onClick(View v) {
            ...
            if (position != RecyclerView.NO_POSITION) { // Check if an item was deleted, but the user clicked it before the UI removed it
                DataCovid dataCovid = getItem(position);
